...
class ReportManager:
    COLUMNS = ['Methodology', 'Name', 'Total Projects',
               'PDD Number', 'First of its Kind', 'Laws and Regulations',
               ]
    ...
    def add_to_dataframe(self, methodology, name, total_projects, pdd_number,
        ...
        if self.is_trace:
            self.df.loc[self.counter, 'First of its Kind'] = f'Y ({step_0_page_number})' if step_0 else 'N'
            self.df.loc[self.counter, 'Laws and Regulations'] = f'Y ({step_1_page_number})' if step_1 else 'N'
        else:
            self.df.loc[self.counter, 'First of its Kind'] = 'Y' if step_0 else 'N'
